...
public class DSPacket {
    ...
    public DSPacket(byte type, int seqNum, byte[] data) {
        ...
        if (type == TYPE_DATA) {
            if (data == null || data.length <= 0 || data.length > MAX_PAYLOAD_SIZE) {
                throw new IllegalArgumentException("DATA payload must be 1..124 bytes.");
            }
            this.payload = data;
        } else {
    ...
    public DSPacket(byte[] rawBytes) {
        ...
        if (this.type != TYPE_SOT &&
            ...
            this.type != TYPE_ACK &&
            this.type != TYPE_EOT) {
            throw new IllegalArgumentException("Invalid packet type: " + this.type);
        }

        // Validate payload length range
        if (this.length < 0 || this.length > MAX_PAYLOAD_SIZE) {
            throw new IllegalArgumentException("Invalid payload length: " + this.length);
        }

        // Control packets must have length 0
        if ((this.type == TYPE_SOT || this.type == TYPE_ACK || this.type == TYPE_EOT) && this.length != 0) {
            throw new IllegalArgumentException("Control packets must have length 0.");
        }

        // DATA packets should have a real payload
        if (this.type == TYPE_DATA && this.length <= 0) {
            throw new IllegalArgumentException("DATA packets must have length 1..124.");
        }
